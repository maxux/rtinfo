...
int main(void) {	
	...
	while(1) {	
		...
		printf("%4lld Mo (%.0f%%)" COLOR_NONE, memory.ram_used / 1024, ((float) memory.ram_used / memory.ram_total) * 100);
		
		printf(" | SWAP: %3lld Mo (%.0f%%)", (memory.swap_total - memory.swap_free) / 1024, ((float) (memory.swap_total - memory.swap_free) / memory.swap_total) * 100);
